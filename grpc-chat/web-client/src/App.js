...
// Adjust these import paths to match your actual generated file locations
...
function App() {
    const [userName, setUserName] = useState(""); // Input for registration
    const [myUser, setMyUser] = useState(null);    // The registered user object
    const [users, setUsers] = useState([]);       // All users from backend
    const [targetUser, setTargetUser] = useState(null); // Selected recipient
    ...
    const startListening = async (userId) => {
    ...
    const handleRegister = async (e) => {
        ...
        try {
            ...
            await fetchUsers(); // Refresh list to show yourself
    ...
    const sendMessage = async (e) => {
        e.preventDefault();
        if (!inputText || !targetUser) return;
        ...
        <div style={styles.appContainer}>
            ...
            <div style={styles.chatArea}>
                ...
                <div style={styles.messageBox}>
                    {messages
                        .filter(m => !targetUser || m.from === targetUser.id || m.from === "Me")
                        .map((m, i) => (
                            <div key={i} style={{
                                ...styles.messageBubble,
                                alignSelf: m.from === "Me" ? "flex-end" : "flex-start",
                                backgroundColor: m.from === "Me" ? "#4f46e5" : "#f3f4f6",
                                color: m.from === "Me" ? "white" : "black",
                            }}>
                                <strong>{m.from === "Me" ? "" : `${m.from}: `}</strong>{m.text}
                            </div>
                        ))}
                </div>
